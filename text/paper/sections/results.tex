...
discretionary spend (top row) and monthly savings (bottom row) under the
unconditional (left column) and conditional (right column) parallel trends
assumptions. As discussed in Section~\ref{sec:estimation}, the conditional
parallel trends assumption states that the outcome variable of treatment and
control units with the same set of covariates would have evolved in parallel
fashion. Throughout my analysis, the set of covariates I use are month income,
month spend, the number of active accounts, and age. Estimates are group-time
average treatment effects aggregated by time since treatment exposure, as
defined in Equation~\ref{eq:att_es}. All results are presented with a uniform
95\% confidence band, based on bootstrapped standard errors that account for
...
\begin{figure}[h] \centering \caption{Main results} \label{fig:main_results}
  ...
  \fignote{\textwidth}{The effect of app use on monthly discretionary
    spending (top row) and monthly savings (bottom row) under the
    unconditional (left column) and conditional (right column) parallel
    trends assumption. Point estimates represent group-time average
    treatment effects aggregated to periods since treatment exposure, as
    defined in Section~\ref{sec:estimation}. Red lines represent point
    estimates and uniform 95\% confidence bands for pre-treatment periods
    allowing for clustering at the user level. If the null hypothesis that
parallel trends hold in all periods is correct, these should be equal to zero.
...
parallel trends are important in contexts where (i) there are covariate specific
...
Finally, we can see that, in contrast to discretionary spending, savings do not
change once users start using the app. Confidence intervals are much larger
than for discretionary spend because -- as can be seen in
Table~\ref{tab:sumstats} -- savings account inflows are often zero, which leads
to a standard deviation much larger than that for discretionary
spend.

The absence of an increase in savings is somewhat surprising, since we might
...
app use rather than also labelling historical transactions. This could happen
in two ways: either users can see the entire account history available to MDB
after linking an account but only label transactions during periods of active
app use and the period immediately before signup, or MDB only shows users data
from the month before signup onwards, so that users are unable to manually
label transactions in earlier periods.
...
Explanations (ii) and (iii) seem plausible and would make for interesting
future research. But why would users just let accumulate funds in their current
accounts instead of moving funds into higher-interest savings accounts? One
possible explanation is that people intend to make these transfers but that
intertia and procrastination from following through. Another explanation is
that historically low interest rates during the entire sample period did not
provide much of an incentive to save in the first place. Both of these
explanations could be explored futher in future researcher.
...
\begin{figure}[h]
    ...
    \fignote{\textwidth}{Effect of app use on total discretionary spend (for
        reference) on the left and discretionary spend paid via debit-direct
